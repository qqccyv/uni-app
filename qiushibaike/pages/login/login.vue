	...
	<view>
		...
		<view class="body">
			<template v-if="loginStatus">
				...
				placeholder="昵称/手机号/邮箱" v-model="userName"/>
			...
			<template v-else>
				<view class="login-input-box">
					...
					placeholder="手机号" v-model="phoneNum" />
				</view>
				<view class="login-input-box">
					<input type="text"
					class="uni-input common-input forget-input"
		...
		<view class="login-status u-f-ajc login-padding login-font-color" @tap="changeLoginStatus">
			{{loginStatus? '验证码登录':'账号密码登录'}}<view class="icon iconfont icon-jinru login-font-color"></view>
		</view>
